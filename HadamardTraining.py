...
from Hadamard_Network import HadamardConv2d
...
model = HadamardConv2d(device).to(device).to(torch.float32)
...
for epoch in range(num_epochs):
    if epoch % 99 == 0:
	    a = model.state_dict()
	    print(a.keys())
	    torch.save(model.state_dict(), model_storage_dir+'hadamard'+str(epoch)+'.ckpt')
